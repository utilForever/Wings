...
public:
  ...
  virtual ~TlsConnection() noexcept;
...
public:
   ...
   * @newin{2,28}
   * 
   * @return  @a conn's certificate, or <tt>0</tt>.
   */
   ...
   * @newin{2,28}
   * 
   * @return  @a conn's peer's certificate, or <tt>0</tt>.
   */
   ...
   * @newin{2,30}
   * 
   * @return The certificate database that @a conn uses or <tt>0</tt>.
   */
   ...
   * g_tls_backend_get_default_database(). If set to <tt>0</tt>, then
   * peer certificate validation will always set the
   * TLS_CERTIFICATE_UNKNOWN_CA error (meaning
   * TlsConnection::signal_accept_certificate() will always be emitted on
   * client-side connections, unless that bit is not set in
   ...
   * for things like prompting the user for passwords. If <tt>0</tt> is returned, then
   ...
   * TlsInteraction. <tt>0</tt> can also be provided if no user interaction
   * should occur for this connection.
   * 
   * @newin{2,30}
   * 
   * @param interaction An interaction object, or <tt>0</tt>.
   */
