...
public:
   ...
   * the value of `TZ`, but if @a identifier is <tt>0</tt> then `/etc/localtime`
   ...
   * the `TZ` environment variable (including the possibility of <tt>0</tt>).
   * 
   * You should release the return value by calling g_time_zone_unref()
   * when you are done with it.
   * 
