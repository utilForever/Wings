...
public:
  ...
  virtual ~DataInputStream() noexcept;
...
# 
   ...
   * error, it will return <tt>0</tt> and @a error will be set.  For UTF-8
   * conversion errors, the set error domain is CONVERT_ERROR.  If
   * there's no content to read, it will still return <tt>0</tt>, but @a error
   * won't be set.
   */
   ...
   * <tt>0</tt> and @a error will be set. For UTF-8 conversion errors, the set
   * error domain is CONVERT_ERROR.  If there's no content to read,
   * it will still return <tt>0</tt>, but @a error won't be set.
   */
